...
function collectImageUrls(article: SeoAgentArticle): string[] {
...
async function saveArticle(article: SeoAgentArticle) {
  ...
  if (!title || !slug || !content) {
    throw new Error('Invalid payload: title, slug, and content are required');
  }

  const imageUrls = collectImageUrls(article);
